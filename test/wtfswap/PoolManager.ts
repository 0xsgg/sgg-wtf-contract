...
describe("PoolManager", function () {
    async function deployFixture() {
        ...
        return { manager, publicClient };   
    }

    it('getPairs & getPools', async function() {
        const { manager, publicClient } = await loadFixture(deployFixture);
        ...
        await manager.write.createAndInitializePoolIfNecessary([{
            ...
            tickUpper: TickMath.getTickAtSqrtRatio(encodeSqrtRatioX96(1000, 1)),
            sqrtPriceX96: BigInt(encodeSqrtRatioX96(100, 1).toString()),
        }]);

        // 由于和前一个参数一样，会合并，不会创建新的池。
        await manager.write.createAndInitializePoolIfNecessary([{
            token0: tokenA,
            token1: tokenB,
            fee: 3000,
    ...
    it("require token0 < token1", async function() {
        ...
        await expect(manager.write.createAndInitializePoolIfNecessary([{
            token0: tokenB,
            token1: tokenA,
            fee: 3000,
            tickLower: TickMath.getTickAtSqrtRatio(encodeSqrtRatioX96(1, 1)),
